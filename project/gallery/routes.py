...
async def show_gallery():
    ...
    return render_template(
        ...
        fav_ids=fav_ids, error=error_msg, is_favorites=False
    )

# --- JSON API (ДЛЯ FLUTTER ПРИЛОЖЕНИЯ) ---
@gallery_bp.route('/api/mobile/gallery')
async def mobile_gallery_api():
    """Этот маршрут будет использовать Flutter приложение"""
    query_tags = request.args.get('tags', '').strip()
    page = int(request.args.get('page', 0))
    sort_mode = request.args.get('sort', 'date')
    ...
    tags_tuple = tuple(query_tags.split()) if query_tags else tuple()
    # Во Flutter черный список будем хранить на телефоне и передавать сюда при необходимости,
    # или фильтровать на клиенте. Пока оставим пустым.
    user_blacklist_tuple = tuple() 

    try:
        posts = await get_posts(tags_tuple, page, sort_mode, user_blacklist_tuple, 20)
...
def toggle_favorite():
    # Этот роут используется только веб-сайтом.
    # Flutter будет хранить избранное ВНУТРИ телефона (SQLite), 
    # поэтому этот роут ему не нужен.
    try:
        data = request.json
        user_ip = get_client_ip()
        ...
        if existing:
            ...
            new_fav = Favorite(user_ip=user_ip, post_id=post_id, file_url=data.get('file_url'),
                               preview_url=data.get('preview_url'), tags=data.get('tags'), media_type=data.get('media_type', 'image'))
...
def show_favorites():
    ...
    posts = [{'id': f.post_id, 'file_url': f.file_url, 'preview_url': f.preview_url, 'type': f.media_type, 'tags': f.tags} for f in favorites]
    return render_template('gallery.html', posts=posts, tags="", sort_mode="date", page=-1, fav_ids=[p['id'] for p in posts], is_favorites=True)
